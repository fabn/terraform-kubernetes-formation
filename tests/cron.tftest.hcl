...
run "api_shaped_pod_template_with_null_optionals" {
  ...
  assert {
    condition     = kubernetes_cron_job_v1.cron.spec[0].job_template[0].spec[0].template[0].spec[0].volume[0].persistent_volume_claim[0].claim_name == "myapp-uploads"
    error_message = "A null-valued source key must not stop the real claim from being inherited"
  }

  assert {
    condition     = length(kubernetes_cron_job_v1.cron.spec[0].job_template[0].spec[0].template[0].spec[0].volume[0].empty_dir) == 0
    error_message = "A claim whose sibling source keys are null must not be rendered as an emptyDir"
  }

  assert {
    condition     = kubernetes_cron_job_v1.cron.spec[0].job_template[0].spec[0].template[0].spec[0].container[0].volume_mount[0].read_only == false
