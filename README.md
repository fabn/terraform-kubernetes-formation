...
| `termination_grace_period_seconds` | Seconds between SIGTERM and SIGKILL (null = k8s default 30). Unlike a PDB, which only covers *voluntary* disruptions, this applies to every termination path — autoscaler scale-in, node drain, Spot reclaim, rollout — so it is what lets a queue worker finish the job in its hands. The worker's own drain timeout (Sidekiq `-t`) must fit inside it. Raising it delays every voluntary drain and consolidation by the same amount, and on Spot the interruption notice (2 min on AWS) is a hard ceiling. | `number` | `null` |
